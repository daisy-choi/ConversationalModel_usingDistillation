...
class Translator(object):
    ...
    def translate(self,
        ...
        import pickle
        #with open('temp_files/test_emb_{}.pkl'.format(self.model_num.replace("txt", "")), 'wb') as f:
        #with open('distill_files/input_emb_{}.pkl'.format(self.model_num.replace("txt", "")), 'wb') as f:
        with open('{}'.format(self.model_num.replace(".txt", "_emb.pkl")), 'wb') as f:
    ...
    def _translate_batch(self, batch, data):
        ...
        if data_type == 'text':
            _, src_lengths = batch.src
        #print (src_lengths)
        src_lengths = torch.LongTensor([max(1, value) for value in src_lengths]).cuda()
